...
describe('page objects through the writer', () => {
  it('moves an image 10 pt, keeps every other byte, and re-renders only there', async () => {
    ...
    try {
      ...
      content = (await pdfium.pageContent(doc, 0)).content;
    ...
    const saved = (await readPageContent(result.bytes, 0))?.content;
  ...
  it('restores the original resources when the base was regenerated by PDFium', async () => {
    // What the app actually hands the writer: the engine's own save after the edit, in which
    // `FPDFPage_GenerateContent` has renamed every resource (/FXX1, /FXF1, …).
    const doc = pdfium.openSync(fixture('image.pdf'));
    let objects: ReadonlyArray<PageObject>;
    let captured: { content: Uint8Array; resources: string };
    let base: Uint8Array;
    try {
      objects = await pdfium.pageObjects(doc, 0);
      captured = await pdfium.pageContent(doc, 0);
      const image = objects.find((o) => o.kind === 'image');
      if (!image) throw new Error('no image');
    ...
    const image = objects.find((o) => o.kind === 'image');
    if (!image) throw new Error('no image');
    expect(captured.resources).toContain('/XObject');
    const plan: WritePlan = {
      ...emptyWritePlan(1),
      pages: [
        {
          source: 0,
          objects: {
            ...baseline(captured.content, objects),
    ...
    const result = await write(base, plan);
    expect(result.warnings).toEqual([]);
    const reopened = pdfium.openSync(result.bytes);
    try {
      const after = await pdfium.pageObjects(reopened, 0);
      expect(after.map((o) => o.kind)).toEqual(objects.map((o) => o.kind));
      expect(after[image.index]?.rect.x0).toBeCloseTo(image.rect.x0 + 10, 2);
    } finally {
      await pdfium.close(reopened);
    }
    // Without the resources the same base loses its images — which is why they travel.
    const bare = await write(base, {
      ...emptyWritePlan(1),
      pages: [
        {
          source: 0,
    ...
    const lost = pdfium.openSync(bare.bytes);
    try {
      expect((await pdfium.pageObjects(lost, 0)).some((o) => o.kind === 'image')).toBe(false);
    } finally {
      await pdfium.close(lost);
    }
  });
