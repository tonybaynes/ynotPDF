...
import { renderStampPdf } from '@engine/appearance/stampPdf';
import {
  appearanceInput,
  applyMatrix,
...
describe('the catalogue', () => {
  ...
  it('every stamp in the catalogue renders to a one-page vector PDF, the same bytes every time', async () => {
    // The files themselves are generated by `npm run stamps` and never committed (the repository
    // refuses PDFs outside test/fixtures/); when one is on this machine it must match the render.
    for (const stamp of catalogue.stamps) {
      const bytes = await renderStampPdf(stamp);
      expect(Buffer.from(bytes.subarray(0, 5)).toString('latin1'), stamp.id).toBe('%PDF-');
      const doc = await PDFDocument.load(bytes);
      expect(doc.getPageCount(), stamp.id).toBe(1);
      const drawing = stampDrawing(stamp.lines, stamp.color);
      expect(doc.getPage(0).getSize(), stamp.id).toEqual({
        width: drawing.width,
        height: drawing.height,
      });
      const again = await renderStampPdf(stamp);
      expect(Buffer.compare(Buffer.from(bytes), Buffer.from(again)), stamp.id).toBe(0);
      const onDisk = join(process.cwd(), 'resources', 'stamps', `${stamp.id}.pdf`);
      if (existsSync(onDisk))
        expect(Buffer.compare(readFileSync(onDisk), Buffer.from(bytes)), stamp.id).toBe(0);
    }
  });
