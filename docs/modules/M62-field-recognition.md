# M62 — Automatic form-field recognition

| | |
|---|---|
| **Module id** | `M62` — folder `src/renderer/modules/M62-field-recognition/`, branch `mod/M62-field-recognition` |
...
You are building **M62 — Automatic form-field recognition** of ynotPDF. Carry this brief out end to
...
2. Create branch `mod/M62-field-recognition` from `main` in a new git worktree and
   work there.
...
Detect likely form fields on a static form (underlines, boxes, check
squares, labels) and create AcroForm fields with sensible names and types
for the operator to review.

## Foxit 14 reference — what to emulate

Foxit Form → Run Form Field Recognition; Designer Assistant highlights
candidates as you hover.

## Scope — build all of this

- Heuristics over `pageObjects` paths + `textRuns`: horizontal lines and
  rectangles → text fields; small squares → checkboxes; circles → radios;
  label text to the left/above → name and tooltip; column of boxes with
  same label → radio group; date-like labels → date fields; signature
  labels → signature fields; table rows → repeated fields with index names.
- Optional raster fallback on scanned forms (after M90 OCR text) using
  line detection on the bitmap.
- Review dialog: list of candidates with type/name/page, toggle each,
  rename, then create as one transaction; "Designer assistant" hover mode
  that proposes one field at a time.
- Accuracy harness with 10 labelled sample forms (synthetic + public
  domain government forms).

## Out of scope

ML models (keep it heuristic; revisit if accuracy is poor).

## Design notes & constraints

- Pure functions over geometry; tune with the harness; log precision/
  recall in the Build log.

## Files you will create or touch

`src/renderer/modules/M62-field-recognition/**`, `test/fixtures/forms/**`,
tests.
...
- ≥ 80 % of expected fields detected with correct type on the labelled
  set; ≤ 10 % false positives.
- Created fields fill and save normally (M60/M61 tests pass on the
  output).
