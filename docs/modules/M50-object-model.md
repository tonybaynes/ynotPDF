...
1. **Two paths to the file, one per page (ADR 0018).** PDFium's page-object API drives
   the *live view*; the object mutations never call `FPDFPage_GenerateContent`, so the
   bytes the engine saves still carry the original content stream and the writer
   replays the edit onto it with our own parser. Unknown operators therefore survive a
   move, a resize, a rotate, a flip and a delete. Z-order and grouping are the
   exception: they need each moved object's whole graphics state re-established, so
   those pages are regenerated by PDFium and the writer plans nothing for them. One
   page never takes both paths. *(ours; forced by the "never lose unknown operators"
   constraint in this brief.)*
...
4. **A clip set outside the object does not move with it.** Wrapping only the object's
   ...
   and what PDFium and Foxit both do. The properties panel says "clipped" on such an
   object so the behaviour is not a surprise. *(spec + Foxit.)*
...
12. **Everything is a command.** `object.*` ids, all in the palette, with the shortcuts
    Foxit uses where they do not collide: arrows nudge, `Mod+G`/`Mod+Shift+G` group,
    `Mod+Alt+[`/`]` send/bring, `Mod+D` duplicate. Conflicts are checked by
    `test/unit/shortcut-conflicts.test.ts`. *(ours + Foxit.)*
