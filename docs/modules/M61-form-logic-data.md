# M61 — Field logic — validation, formatting, calculation, actions; data import/export; flatten

| | |
|---|---|
| **Module id** | `M61` — folder `src/renderer/modules/M61-form-logic-data/`, branch `mod/M61-form-logic-data` |
| **Earliest wave** | 6 (see `PLAN.md` §0/§12) |
| **Tier** | Core |
| **Depends on** | M60 |
| **Unlocks** | [M41 Merge, split, extract to files, crop & flatten](./M41-merge-split-crop.md), [M120 Batch processing & action wizard](./M120-batch-actions.md) |

## Your task — the prompt for this conversation

You are building **M61 — Field logic — validation, formatting, calculation, actions; data import/export; flatten** of ynotPDF. Carry this brief out end to
end without waiting to be asked for the next step:

1. Read this file completely, then `CLAUDE.md`, `PLAN.md` §2–4, §9 and §12,
   and the briefs of your direct dependencies: [M60 Form fill & AcroForm field designer](./M60-forms.md).
   Confirm each dependency is ☑ in `PLAN.md` §0. If one is not, say so and
   stop — do not build against unfinished work.
2. Create branch `mod/M61-form-logic-data` from `main` in a new git worktree and
   work there.
...
The behaviour behind fields: formats, validation, calculations, and
actions, implemented with a safe in-house expression engine (Acrobat
JavaScript is Parked); form data import/export in FDF/XFDF/CSV/XML; reset
and flatten.

## Foxit 14 reference — what to emulate

Foxit field properties Format (number/percent/date/time/special/custom),
Validate (range, custom), Calculate (sum/product/avg/min/max of fields,
simplified field notation, custom JS ✗), Actions (mouse/keyboard triggers:
go to page, open URL/file, reset form, submit form, show/hide field, run
JS ✗, play sound ✗), Import/Export form data, Reset Form, Flatten.

## Scope — build all of this

- Expression engine (`engine/forms/expr/`): parse Acrobat *simplified
  field notation* (`+ - * / ( )` over field names) and a defined function
  set (sum/product/avg/min/max/if/round/…); typed values; recalculation
  order (`/CO`) respected; runs on value change; never executes arbitrary
  code. Also translate the common `AFNumber_Format`, `AFDate_FormatEx`,
  `AFSimple_Calculate`, `AFRange_Validate`, `AFSpecial_Format`, `AFPercent_Format`
  patterns found in existing PDFs into our engine (regex-recognised) so
  Acrobat-authored forms behave; unknown JS → field marked "custom script
  not supported" (word + icon) and left as-is.
- Format/validate/calculate UI in the properties dialog writing standard
  `/AA` entries *and* the equivalent Acrobat JS strings so the form also
  works in Acrobat/Foxit.
- Actions editor: triggers (MouseUp/Down/Enter/Exit, Focus/Blur, page
  open/close, doc open) × actions (GoTo, GoToR, URI, Launch (confirm),
  ResetForm, SubmitForm (HTML/FDF/XFDF/PDF, via IPC fetch with confirmation),
  Hide, Named (Print/NextPage…), SetOCGState); shared with M53/M12.
- Data: export FDF/XFDF/CSV/XML (Foxit layouts), import (merge by name),
  batch export of many files to one CSV (M120 hook).
- Reset form (all/selected), flatten (writer op used by M41/M21 Save As).

## Out of scope

JavaScript execution. Distribution/collection workflows (Parked).

## Design notes & constraints

- Expression engine is pure and fuzz-tested; a formatting mistake in money
  fields is a real-world bug, so cover en-GB/decimal/thousands cases.

## Files you will create or touch

`src/renderer/modules/M61-form-logic-data/**`, `src/engine/forms/expr/**`,
`src/engine/forms/data/**`, tests.
...
- Calculation fixture (invoice with 20 lines) totals match the expected
  spreadsheet values; export CSV equals expected.
- Acrobat-authored `AFSimple_Calculate` forms compute in our app.
- Validation rejects out-of-range with a worded message; format shows
  1,234.50 for 1234.5.
- Import XFDF fills every field; flatten produces identical render with no
  fields.
