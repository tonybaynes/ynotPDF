# M110 — Compare documents

| | |
|---|---|
| **Module id** | `M110` — folder `src/renderer/modules/M110-compare/`, branch `mod/M110-compare` |
...
You are building **M110 — Compare documents** of ynotPDF. Carry this brief out end to
...
2. Create branch `mod/M110-compare` from `main` in a new git worktree and
   work there.
...
Compare two PDFs visually and textually, present results side by side
with synced scrolling and a change list, and produce a comparison report
PDF.

## Foxit 14 reference — what to emulate

Foxit View → Compare: choose old/new, compare pages/text/annotations,
results view with markup colours (we use blue/yellow + labels), change
list panel with next/previous, report.

## Scope — build all of this

- Compare dialog: two documents (open tabs or files), page ranges/offset,
  mode (text, visual, both), sensitivity, ignore annotations.
- Text diff: per page reading-order text via M13 grouping, diff-match-patch
  at word level, mapped back to glyph boxes for highlighting; moved-text
  detection basic.
- Visual diff: render both at same DPI, align pages, pixel diff with
  tolerance and blur, cluster into regions.
- Results: side-by-side view (two viewports, synced), overlays with
  labels "Added"/"Removed"/"Changed" (blue/yellow tokens + icons, never red/
  green), change list panel with filters, next/prev, summary counts.
- Report: PDF with side-by-side thumbnails and a change table (pdf-lib).

## Out of scope

Word-level formatting diffs; annotation diffs beyond presence.

## Design notes & constraints

- Diff runs in a worker; results are plain data so tests assert counts.

## Files you will create or touch

`src/renderer/modules/M110-compare/**`, `src/engine/compare/**`, tests.
...
- Fixture pair with exactly 3 known text changes and 1 image change ⇒
  report lists 3 text + 1 visual; identical pair ⇒ zero.
