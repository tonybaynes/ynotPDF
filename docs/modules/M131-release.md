# M131 — Installers, code signing, auto-update, crash reporting, help & first run

| | |
|---|---|
| **Module id** | `M131` — folder `src/renderer/modules/M131-release/`, branch `mod/M131-release` |
| **Earliest wave** | 10 (see `PLAN.md` §0/§12) |
| **Tier** | Core |
| **Depends on** | M00, M130 |
| **Unlocks** | — |
...
You are building **M131 — Installers, code signing, auto-update, crash reporting, help & first run** of ynotPDF. Carry this brief out end to
end without waiting to be asked for the next step:

1. Read this file completely, then `CLAUDE.md`, `PLAN.md` §2–4, §9 and §12,
   and the briefs of your direct dependencies: [M00 Scaffold, CI & packaging smoke](./M00-scaffold.md), [M130 Preferences, keyboard shortcuts editor, ribbon/QAT customisation, UI scale, i18n framework](./M130-preferences.md).
   Confirm each dependency is ☑ in `PLAN.md` §0. If one is not, say so and
   stop — do not build against unfinished work.
2. Create branch `mod/M131-release` from `main` in a new git worktree and
   work there.
...
Ship it: signed installers for all three OSes, auto-update channel,
crash/log reporting, help content, About, first-run experience, and the
release checklist.

## Foxit 14 reference — what to emulate

Foxit's installer/updater/help/about equivalents.

## Scope — build all of this

- electron-builder targets: Windows NSIS (per-user + per-machine) and MSI,
...
- Auto-update: electron-updater with GitHub Releases (private repo needs a
  ...
  notes dialog (opaque), deferred install.
- Logging: rotating logs in userData, "Open log folder", crash reporter
  writing minidumps locally; optional upload OFF by default (privacy).
...
- Release checklist `docs/release.md`: version bump, changelog, tag, CI
  publish, smoke on all three OSes, update feed check.
- Telemetry: none.

## Out of scope

App stores (MS Store / Mac App Store) — later if wanted.
...
- **Windows on ARM (M03):** ship NSIS + MSI for both `x64` and `arm64`;
  ...
  the download page/first-run must name which one the user has.
- Keep unsigned builds working for local testing (`CSC_IDENTITY_AUTO_
  DISCOVERY=false`).

## Files you will create or touch

`electron-builder.yml`, `.github/workflows/release.yml`, `src/main/
updater/**`, `src/main/logging/**`, `docs/help/**`, `docs/release.md`,
`build/` (icons, entitlements, installer scripts).

## Libraries

electron-updater, electron-log (MIT).

## Acceptance tests — the module is done when these pass on all three OSes

- Tagged release builds signed installers for all three OSes (or unsigned
  with a clear CI warning until certificates exist); installing v0.9 then
  publishing v1.0 ⇒ the app offers and applies the update on each OS
