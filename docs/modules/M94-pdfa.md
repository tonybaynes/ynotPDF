# M94 — PDF/A conversion & basic validation

| | |
|---|---|
| **Module id** | `M94` — folder `src/renderer/modules/M94-pdfa/`, branch `mod/M94-pdfa` |
...
You are building **M94 — PDF/A conversion & basic validation** of ynotPDF. Carry this brief out end to
end without waiting to be asked for the next step:

1. Read this file completely, then `CLAUDE.md`, `PLAN.md` §2–4, §9 and §12,
   and the briefs of your direct dependencies: [M100 Optimise (reduce size), linearise, repair, remove duplicates](./M100-optimise-repair.md), [M72 Document properties, metadata & XMP, initial view](./M72-properties-metadata.md).
   Confirm each dependency is ☑ in `PLAN.md` §0. If one is not, say so and
   stop — do not build against unfinished work.
2. Create branch `mod/M94-pdfa` from `main` in a new git worktree and
   work there.
...
Convert documents to PDF/A-1b, 2b, 3b (fonts embedded, output intent,
XMP conformance metadata, forbidden features removed or flattened) and
run a basic in-house conformance check; full validation via veraPDF only
as a CI check.

## Foxit 14 reference — what to emulate

Foxit Convert → PDF/A, PDF/E, PDF/X (we do PDF/A only); Preflight ✗.
...
- CI: run veraPDF (GPL/MPL — a *tool run in CI only*, never shipped) to
  assert outputs pass.

## Out of scope

PDF/A-a (accessible) levels, PDF/E, PDF/X, full preflight.

## Design notes & constraints

- Keep a clear report of what conversion changed.

## Files you will create or touch

`src/engine/pdfa/**`, `src/renderer/modules/M94-pdfa/**`, `resources/icc/`,
tests.

## Libraries

None new (sRGB ICC profile — verify licence).
