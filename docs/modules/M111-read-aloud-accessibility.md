# M111 — Read aloud, accessibility check & alt text

| | |
|---|---|
| **Module id** | `M111` — folder `src/renderer/modules/M111-read-aloud-accessibility/`, branch `mod/M111-read-aloud-accessibility` |
...
You are building **M111 — Read aloud, accessibility check & alt text** of ynotPDF. Carry this brief out end to
end without waiting to be asked for the next step:

1. Read this file completely, then `CLAUDE.md`, `PLAN.md` §2–4, §9 and §12,
   and the briefs of your direct dependencies: [M13 Text selection, find, copy, snapshot & print](./M13-select-find-print.md).
   Confirm each dependency is ☑ in `PLAN.md` §0. If one is not, say so and
   stop — do not build against unfinished work.
2. Create branch `mod/M111-read-aloud-accessibility` from `main` in a new git worktree and
   work there.
...
Read the document aloud with OS text-to-speech and follow-along
highlighting; run a quick accessibility check; view reading order; edit
alt text for images (structure-tree aware where present).

## Foxit 14 reference — what to emulate

Foxit View → Read: Read Current Page / From Current Page, pause/stop,
rate/volume/voice; Accessibility tab: Full Check (we do a quick check),
Reading Order tool, Set Alternate Text, Autotag ✗, Tags panel ✗ (read-only
view of existing tags is fine).

## Scope — build all of this

- Read aloud via `speechSynthesis` (voices from OS), sentence chunking
  from M13 text grouping, highlight current sentence (selection token),
  controls in a floating opaque bar and shortcuts; read selection.
- Quick accessibility check: tagged?, `/Lang`, title, images with alt text,
  form fields with tooltips, tab order set, colour contrast of text vs
  page background (sampled), scanned-only pages — results panel with
  words + icons and jump links; export report.
- Reading order view: numbered overlays from structure tree if present,
  else from heuristic grouping; allow reordering heuristics per page
  (stored in `custom`, used by read aloud/export).
- Alt text editor: for `/Figure` structure elements when tagged; for
  untagged, store in `custom` and write as a minimal tag on the image when
  saving (create `/StructTreeRoot` if absent — scoped to figures).
- Tags panel: read-only tree view of the structure tree.

## Out of scope

Full tagging/autotag, tag editing (Parked).
...
`src/renderer/modules/M111-read-aloud-accessibility/**`,
`src/engine/structure/**`, tests.
...
- Read aloud speaks the fixture's first sentence (e2e checks the utterance
  text and highlight movement).
- Check reports the seeded issues (no title, image without alt, no lang).
- Alt text set on a figure in the tagged fixture persists and is read
  back via the structure tree.
