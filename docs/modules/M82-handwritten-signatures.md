# M82 — Handwritten signatures & initials
...
You are building **M82 — Handwritten signatures & initials** of ynotPDF. Carry this brief out end to
...
Foxit's quick-sign: create signature appearances by drawing, typing,
importing an image or using the webcam; place them anywhere or into
signature fields; manage a saved set; initials; "sign all pages".

## Foxit 14 reference — what to emulate

Foxit Fill & Sign: Create Signature (draw/type with fonts/import/paste/
camera), password-protect signatures, Place, resize, initials, Apply all,
manage signatures.

## Scope — build all of this

- Signature creator dialog (opaque): draw (pressure, smoothing from M31),
  type (bundled script-style fonts under `resources/fonts/signature/`),
  import image (auto-trim white, threshold to black/blue), webcam capture
  via `getUserMedia` (permission prompt), colour presets (black/blue via
  tokens), save name + optional encrypt with a passphrase (settings store,
  encrypted at rest).
- Placement: click to place, drag to size/move, rotate; as a stamp-like
  annotation (Foxit uses a stamp with a marker) or flattened into content
  (setting) or into an empty signature field as its appearance (without a
  digital signature — clearly worded); "apply to all pages" / positions
  preset (bottom-right…).
- Initials variant; recent/favourites strip in the ribbon gallery.

## Out of scope

Digital (cryptographic) signing (M81).

## Design notes & constraints

- Signature images are vector when drawn/typed (paths), raster when
  imported; both go through `engine/appearance/`.

## Files you will create or touch

`src/renderer/modules/M82-handwritten-signatures/**`, `resources/fonts/
signature/**`, tests.
...
- Create each kind, place on 3 pages, save, reopen ⇒ present; flattened
  variant has no annotation and renders identically.
- Encrypted saved signature cannot be used without the passphrase.
