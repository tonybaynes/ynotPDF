# M54 — Find & replace, spell-check

| | |
|---|---|
| **Module id** | `M54` — folder `src/renderer/modules/M54-find-replace-spellcheck/`, branch `mod/M54-find-replace-spellcheck` |
...
You are building **M54 — Find & replace, spell-check** of ynotPDF. Carry this brief out end to
end without waiting to be asked for the next step:

1. Read this file completely, then `CLAUDE.md`, `PLAN.md` §2–4, §9 and §12,
   and the briefs of your direct dependencies: [M51 Text editing with reflow (spike first)](./M51-text-editing.md), [M13 Text selection, find, copy, snapshot & print](./M13-select-find-print.md).
   Confirm each dependency is ☑ in `PLAN.md` §0. If one is not, say so and
   stop — do not build against unfinished work.
2. Create branch `mod/M54-find-replace-spellcheck` from `main` in a new git worktree and
   work there.
...
Replace text across the document using M51's writer, and spell-check
document text and annotations with en-GB by default.

## Foxit 14 reference — what to emulate

Foxit Find & Replace (Ctrl+H: match case, whole word, replace/replace all,
across all open docs); Spell Check (Comment text, form fields, text edits;
suggestions, ignore/add to dictionary; underline while typing in
annotations).

## Scope — build all of this

- Replace UI on the find bar: replace one / all, scope current/all open,
  options as find; preview count; each replace is a Command via M51's
  block editor (reflow rules apply); report of skipped occurrences (font
  lacks glyphs) with reasons.
- Spell-check service (worker): nspell + en-GB Hunspell dictionary bundled;
  extra dictionaries downloadable to userData; user dictionary; check
  document text (report panel with jump/replace), annotations and form
  values; live squiggle (solid, not translucent) in annotation/free-text
  editing overlays; suggestions context menu.

## Out of scope

Grammar. OCR correction UI (M90).

## Design notes & constraints

- Replace never touches pages without matches.
- Dictionaries are data under `resources/dictionaries/`.

## Files you will create or touch

`src/renderer/modules/M54-find-replace-spellcheck/**`,
`resources/dictionaries/**`, tests.
...
nspell (MIT), dictionary-en-gb (MIT/Hunspell licences vary — verify).

## Acceptance tests — the module is done when these pass on all three OSes

- Replace all 20 occurrences in the fixture ⇒ extraction confirms; undo
  restores byte-identical pages.
- Spell-check flags the 5 seeded misspellings and none of the correct
  words; add-to-dictionary persists.
