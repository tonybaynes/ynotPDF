# M121 — Command-line interface

| | |
|---|---|
| **Module id** | `M121` — folder `src/renderer/modules/M121-cli/`, branch `mod/M121-cli` |
...
You are building **M121 — Command-line interface** of ynotPDF. Carry this brief out end to
end without waiting to be asked for the next step:

1. Read this file completely, then `CLAUDE.md`, `PLAN.md` §2–4, §9 and §12,
   and the briefs of your direct dependencies: [M120 Batch processing & action wizard](./M120-batch-actions.md).
   Confirm each dependency is ☑ in `PLAN.md` §0. If one is not, say so and
   stop — do not build against unfinished work.
2. Create branch `mod/M121-cli` from `main` in a new git worktree and
   work there.
...
`ynotpdf` command-line entry sharing the engine and batch operations for
scripting and integration.

## Foxit 14 reference — what to emulate

Foxit has no full CLI; this is our advantage. Model on qpdf/pdftk verbs.

## Scope — build all of this

- Entry `ynotpdf` (installed alongside the app; Windows `.cmd` shim, mac/
  Linux symlink offered by installer) running Electron in headless mode
  (no window) or a plain Node bundle of `src/engine` + `src/main/batch`
  (preferred — no GUI dependency; decide in ADR).
- Verbs: `info`, `merge`, `split`, `extract`, `rotate`, `ocr`, `encrypt`,
  `decrypt`, `watermark`, `header`, `bates`, `optimise`, `pdfa`, `to-image`,
  `to-text`, `from-images`, `from-office`, `export-data`, `import-data`,
  `flatten`, `redact --pattern`, `sign`, `verify`, `run-action <file.json>`;
  consistent flags, `--json` output, exit codes, `--help` per verb.
- Docs: `docs/cli.md` generated from the verb registry.
...
- Reuse the batch op registry: verbs are generated from it plus a few
  primitives.

## Files you will create or touch

`src/cli/**`, `docs/cli.md`, tests (spawn the CLI in vitest).
...
- `ynotpdf merge a.pdf b.pdf -o out.pdf` equals GUI combine byte-wise
  (after normalising IDs); `ynotpdf info --json` on corpus matches the
  fixture manifest; non-zero exit and message on a bad input.
