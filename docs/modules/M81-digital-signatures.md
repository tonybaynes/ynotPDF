# M81 — Digital signatures — sign, certify, timestamp, LTV, validate, trust

| | |
|---|---|
| **Module id** | `M81` — folder `src/renderer/modules/M81-digital-signatures/`, branch `mod/M81-digital-signatures` |
...
You are building **M81 — Digital signatures — sign, certify, timestamp, LTV, validate, trust** of ynotPDF. Carry this brief out end to
end without waiting to be asked for the next step:

1. Read this file completely, then `CLAUDE.md`, `PLAN.md` §2–4, §9 and §12,
   and the briefs of your direct dependencies: [M80 Incremental-update writer](./M80-incremental-writer.md), [M60 Form fill & AcroForm field designer](./M60-forms.md).
   Confirm each dependency is ☑ in `PLAN.md` §0. If one is not, say so and
   stop — do not build against unfinished work.
2. Create branch `mod/M81-digital-signatures` from `main` in a new git worktree and
   work there.
...
Certificate-based signing with PAdES profiles, timestamping and long-term
validation, certifying with permissions, signature validation with a
managed trust store, digital-ID management across OS stores, and the
Signatures panel.

## Foxit 14 reference — what to emulate

Foxit Protect → Sign & Certify: Place Signature (digital), Certify (visible/
invisible, allowed changes), Sign document with existing field, Digital IDs
(create self-signed, import .pfx/.p12, Windows store / macOS Keychain,
PKCS#11 tokens), Timestamp server settings, Trusted Certificates, Verify
Signature / Signature panel (status words, details, revision view "view
signed version"), Signature appearance designer (name, reason, location,
date, logo, DN, labels), Long-term validation (embed OCSP/CRL).

## Scope — build all of this

- Digital-ID service (main process): list/import `.p12/.pfx`, create
  self-signed (RSA 2048/3072, ECDSA P-256), Windows certificate store via
  `certutil`/PowerShell bridge or `win-ca`-style native access, macOS
  Keychain via `security` CLI, Linux file-based; PKCS#11 (Pro) via `pkcs11js`
  (MIT) later; private keys never leave main; passphrase prompts opaque.
- Signing (`engine/sign/`): compute `/ByteRange` with placeholder, CMS
  SignedData (RSA-PSS/PKCS1, ECDSA, SHA-256/384/512), PAdES-B-B; B-T with
  RFC 3161 timestamp (configurable TSA URL, via IPC fetch); B-LT by
  embedding OCSP/CRL in a DSS; signature field creation or existing field;
  appearance stream from the designer; certify (`/DocMDP` P=1/2/3);
  document-timestamp only signature; multiple signatures via M80.
- Validation: parse signatures, verify hashes/`ByteRange` coverage,
  chain build to trust anchors (app trust store + OS roots via main), OCSP/
  CRL checks (online, cached, offline mode), timestamp validation, DocMDP
  change analysis (what changed after signing — incremental diff), status
  words (Valid / Unknown signer / Modified / Invalid) with icons, Signatures
  panel, "View signed version", details dialog.
- Trust store UI: add/remove trusted certs, trust for signing/certifying,
  import from signature.
- Foxit-style "sign with visible appearance" flow: draw field → choose ID →
  appearance → sign → Save As prompt.

## Out of scope

Cloud signing services, e-signature workflows (Parked). Hardware tokens
in the first pass (mark Pro follow-up).

## Design notes & constraints

- Use `@signpdf` helpers for ByteRange plumbing but own the CMS build with
  `pkijs`/`node-forge` for PAdES attributes (signing-certificate-v2,
  content-type, message-digest, signing-time only where allowed).
- Validate with a *different* implementation in CI (`pdfsig` from poppler or
  Acrobat manually recorded) to avoid self-agreement.

## Files you will create or touch

`src/main/identity/**`, `src/engine/sign/**`, `src/renderer/modules/M81-
digital-signatures/**`, tests, `docs/adr/00NN-signing.md`.
