...
import { extractPageImages } from './imageExtraction';
import { imageAppearances } from './imageAppearances';
...
export class PdfiumEngine implements PdfEngine, CancellableEngine {
  ...
  /** Intrinsic images, including nested forms; ADR 0027 keeps original streams optional. */
  async pageImages(
    doc: DocHandle,
    page: PageIndex,
    ...
    if (options.purpose === 'appearance')
      return imageAppearances(this.ffi, bytes, page, this.geometry(loaded));
    return extractPageImages(
      bytes,
      page,
      (pdf, width, height) => this.decodeImagePdf(pdf, width, height),
      options,
    );
  }

  /** Decode a temporary image-only PDF without touching a live document or its dirty state. */
  private decodeImagePdf(bytes: Uint8Array, width: number, height: number): Uint8Array {
    const ffi = this.ffi;
    return ffi.scope((s) => {
      const data = s.bytes(bytes);
      const docPtr = ffi.call('FPDF_LoadMemDocument', data, bytes.length, 0);
      if (!docPtr) throw new EngineError('corrupt', 'Could not open the isolated image');
      try {
        const pagePtr = ffi.call('FPDF_LoadPage', docPtr, 0);
        if (!pagePtr) throw new EngineError('corrupt', 'Could not open the isolated image page');
        try {
          const obj = ffi.call('FPDFPage_GetObject', pagePtr, 0);
          const bitmap = obj ? this.renderedImage(docPtr, pagePtr, obj) : null;
          if (bitmap?.width !== width || bitmap.height !== height) {
            throw new EngineError('corrupt', 'Could not decode the image at its stored dimensions');
          }
          return bitmap.rgba;
        } finally {
          ffi.call('FPDF_ClosePage', pagePtr);
        }
      } finally {
        ffi.call('FPDF_CloseDocument', docPtr);
      }
    });
  }
  ...
  private renderedImage(
    ...
    const bitmap = ffi.call('FPDFImageObj_GetRenderedBitmap', docPtr, pagePtr, obj);
