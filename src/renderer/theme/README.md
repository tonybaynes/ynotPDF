...
| `gallery.*`                                                            | Dev-only approval page: `npm run gallery` serves <http://localhost:5199/gallery.html>. `--strictPort` makes a port clash fail loudly instead of silently moving to another port and leaving you reading a stale page; `--host 127.0.0.1` keeps it off the network. If 5199 is taken, free it rather than letting the server wander. |
